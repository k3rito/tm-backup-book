...
class TransferService:
    ...
    async def run(self) -> None:
        ...
        # Bolt ⚡ Optimization: Reuse the returned RSS bytes instead of querying twice.
    ...
    async def _run_pipeline(self) -> None:
        ...
        while self._failure is None:
            try:
                ...
                if attempt >= 5:
                    raise
                # Bolt ⚡ Optimization: Reuse RSS bytes instead of querying twice.
                rss = current_rss_bytes()
    ...
    async def _flush_completed(self, force: bool = False) -> None:
        ...
        # Bolt ⚡ Optimization: Batch the state persistence call outside the loop.
        # This converts the progress sync from O(N) network calls and disk writes to O(1)
        # per flush operation, avoiding redundant R2 uploads and heavy local disk I/O.
