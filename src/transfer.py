...
class TransferService:
    ...
    async def _flush_completed(self, force: bool = False) -> None:
        # Optimization (Bolt ⚡): Rather than persisting the state in every single iteration
        # of the loop (O(N) network / disk IO), we batch updates and persist exactly once at the
        # end of the flush operation. This results in O(1) persist calls per flush instead of O(N).
        advanced = False
        while True:
