    ...
    async def _flush_completed(self, force: bool = False) -> None:
        ...
        while True:
            if self._next_commit_id in self._completed_outcomes:
                ...
                # Optimization: Do not persist progress state on every single outcome loop iteration.
                # Batch persist after flushing all consecutive completed/skipped messages to reduce R2/I/O sequential overhead from O(N) to O(1).
