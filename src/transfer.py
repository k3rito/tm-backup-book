...
class TransferService:
    ...
    async def _flush_completed(self, force: bool = False) -> None:
        # Optimization (Bolt ⚡): Batch progress state persistence outside the flush loop.
        # This transforms the state saving from O(N) disk/network writes to O(1) per flush,
        # dramatically reducing redundant Cloudflare R2 upload API calls and local file I/O.
        advanced = False
        while True:
