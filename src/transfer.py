...
class TransferService:
    ...
    async def _flush_completed(self, force: bool = False) -> None:
        ...
        # Performance Optimization: Batch state persistence calls outside the flush loop.
        # Calling _persist_progress_state inside the loop caused O(N) sequential network
        # roundtrips to Cloudflare R2 and disk writes for every message in a flush sequence.
        # Batching outside reduces state persistence calls to O(1) per flush, updating to the latest ID.
