    ...
    async def _flush_completed(self, force: bool = False) -> None:
        ...
        while True:
            if self._next_commit_id in self._completed_outcomes:
                ...
                advanced = True
                # Move state persistence outside the loop to batch I/O and improve performance (O(N) -> O(1))
            ...
            if self._next_commit_id < self._highest_seen_id and self._next_commit_id not in self._seen_ids:
                # Advance progress through message gaps to improve resume efficiency
                self._next_commit_id += 1
                advanced = True
                continue
        ...
        if advanced:
            self._progress_state = ProgressState(last_message_id=self._next_commit_id - 1)
