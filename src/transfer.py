...
class TransferService:
    ...
    async def _flush_completed(self, force: bool = False) -> None:
        # Move state persistence outside the loop to reduce local I/O and Cloudflare R2 API requests
        # from O(N) to O(1) per flush, preventing performance bottlenecks when batching multiple completed messages.
        advanced = False
        while True:
