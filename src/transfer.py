    ...
    async def _flush_completed(self, force: bool = False) -> None:
        ...
        while True:
            if self._next_commit_id in self._completed_outcomes:
                ...
                # Bolt Optimization: Move _persist_progress_state() outside of this loop.
                # Moving it outside batches the progress state updates, reducing R2 and local disk IO
                # from O(N) to O(1) per batch flush, improving transfer efficiency significantly.
