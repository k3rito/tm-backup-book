...
class TransferService:
    ...
    async def run(self) -> None:
        ...
        # Bolt ⚡ Optimization: Query memory RSS once and reuse it for bytes and MB logs.
    ...
    async def _flush_completed(self, force: bool = False) -> None:
        ...
        # Bolt ⚡ Optimization: Batched state persistence. Instead of uploading and saving
        # the progress state on every single iteration of the while loop (O(N) network calls),
        # we batch the updates and persist the state exactly once after the loop finishes (O(1)).
