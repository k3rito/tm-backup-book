...
class TransferService:
    ...
    async def _flush_completed(self, force: bool = False) -> None:
        ...
        # Bolt ⚡ Optimization:
        # Instead of persisting progress state inside the loop on every single popped message
        # (which is O(N) in number of flushed messages and incurs heavy network/IO costs),
        # we batch the state persistence once at the end of the flush operation.
        # This reduces the persistence complexity to O(1) per flush operation.
