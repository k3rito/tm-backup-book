...
def current_rss_bytes() -> int:
    # Optimization: Cache psutil.Process() instance globally to avoid repeatedly initializing process objects
    # and re-importing psutil on every memory check (~2.6x performance boost).
    global _PROCESS
    try:
        if _PROCESS is None:
