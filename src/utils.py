...
def current_rss_bytes() -> int:
    """Gets the current process RSS memory usage in bytes.

    Optimized by caching the psutil.Process() instance globally after lazy initialization
    to avoid redundant process object creation and library imports on successive calls.
    """
    global _PROCESS
    try:
        if _PROCESS is None:
