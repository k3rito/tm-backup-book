...
def current_rss_bytes() -> int:
    """Returns current process Resident Set Size (RSS) memory usage in bytes.

    Optimized by lazily caching the psutil.Process() instance to avoid
    re-instantiating the process object and re-importing psutil on every call (~2.6x speedup).
    """
    global _PROCESS
    try:
        if _PROCESS is None:
